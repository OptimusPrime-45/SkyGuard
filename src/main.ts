...
Sentry.init({
  ...
  release: `skyguard@${__APP_VERSION__}`,
...
Object.defineProperty(window, 'beta', {
  get() {
    const on = localStorage.getItem('skyguard-beta-mode') === 'true';
    console.log(`[Beta] ${on ? 'ON' : 'OFF'}`);
    return on;
  },
  set(v: boolean) {
    if (v) localStorage.setItem('skyguard-beta-mode', 'true');
    else localStorage.removeItem('skyguard-beta-mode');
    location.reload();
  },
});
