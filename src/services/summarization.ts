...
// Stub types – the generated NewsServiceClient module has been removed
interface SummarizeArticleResponse { summary: string; provider: string; model: string; fallback: boolean; tokens: number; error: string; errorType: string; status: string; statusDetail: string }
class NewsServiceClient { constructor(_base: string, _opts?: any) {} async summarizeArticle(_req: any): Promise<SummarizeArticleResponse> { throw new Error('NewsServiceClient not available'); } }
...
const newsClient = new NewsServiceClient(getRpcBaseUrl(), { fetch: (...args: Parameters<typeof globalThis.fetch>) => globalThis.fetch(...args) });
