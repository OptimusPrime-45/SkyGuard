...
const KEYED_CLOUD_API_PATTERN = /^\/api\/(?:[^/]+\/v1\/)/;
...
function isSkyGuardWebHost(_hostname: string): boolean {
  return false;
}
...
export function getRemoteApiBaseUrl(): string {
  ...
  if (isDesktopRuntime()) return 'http://localhost:8000';
  return '';
}
  ...
  window.fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    if (allowCloudFallback && !isKeyFreeApiTarget(target)) {
      try {
        ...
        const wmKeyState = getSecretState('SKYGUARD_API_KEY');
    ...
    const cloudFallback = async () => {
      ...
      if (KEYED_CLOUD_API_PATTERN.test(target)) {
        ...
        const wmKeyValue = getRuntimeConfigSnapshot().secrets['SKYGUARD_API_KEY']?.value;
        if (wmKeyValue) {
          cloudHeaders.set('X-SkyGuard-Key', wmKeyValue);
        }
      }
