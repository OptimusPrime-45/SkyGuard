...
export class DeckGLMap {
  ...
  private radarFlights: any[] = [];
  private onRadarFlightClickCallback: ((flight: any) => void) | null = null;
  ...
  constructor(container: HTMLElement, initialState: DeckMapState) {
    ...
    this.createControls();
    // this.createTimeSlider(); // Removed per refactor
  ...
  private buildLayers(): LayersList {
    ...
    if (this.radarFlights && this.radarFlights.length > 0) {
      layers.push(...this.createRadarFlightsLayers());
    }
  ...
  private createRadarFlightsLayers(): (Layer | null | false)[] {
    if (!this.radarFlights.length) return [];
    const layers: (Layer | null | false)[] = [];

    // 0. Restricted Zone (No-Fly Zone) - Parliament House, New Delhi
    const RESTRICTED_LAT = 28.6172;
    const RESTRICTED_LON = 77.2081;
    const RESTRICTED_RADIUS_M = 3.0 * 1852; // 3 NM in meters

    layers.push(new ScatterplotLayer({
      id: 'radar-restricted-zone',
      data: [{ position: [RESTRICTED_LON, RESTRICTED_LAT] }],
      getPosition: (d: any) => d.position,
      getRadius: RESTRICTED_RADIUS_M,
      getFillColor: [255, 0, 0, 25],
      getLineColor: [255, 68, 68, 160],
      stroked: true,
    ...
    // 1. Full 3-minute trajectory arcs (PathLayer)
    const flightsWithPath = this.radarFlights.filter((f: any) => f.path && f.path.length > 0);
    if (flightsWithPath.length > 0) {
      layers.push(new PathLayer({
        id: 'radar-flights-trajectory',
        data: flightsWithPath,
        getPath: (d: any) => {
          const pts: [number, number][] = [[d.lon, d.lat]];
          for (const wp of d.path) {
            pts.push([wp.lon, wp.lat]);
          }
          return pts;
        },
        getColor: (d: any) => {
          if (d.is_anomaly || d.ml_classification === 'Drone/UAV') return [255, 68, 68, 120];
          return [100, 180, 255, 80];
        },
        getWidth: 2,
        widthMinPixels: 1,
        getDashArray: [6, 4],
    ...
    // 2. Risk halos (ScatterplotLayer)
    layers.push(new ScatterplotLayer({
      id: 'radar-flights-halo',
      ...
      getFillColor: (d: any) => {
        const score = d.anomaly_score ?? 0;
        if (score > 0.85) return [255, 68, 68, 40];
        if (score > 0.7) return [255, 136, 0, 30];
        if (score > 0.5) return [255, 204, 0, 20];
        return [68, 204, 68, 15];
      },
      radiusUnits: 'meters',
      pickable: false,
    ...
    // 3. Single-hop predicted position (PathLayer) - short line to first predicted point
    if (this.radarFlights.some((f: any) => f.predicted_lat && f.predicted_lon)) {
      layers.push(new PathLayer({
        id: 'radar-flights-path',
        data: this.radarFlights.filter((f: any) => f.predicted_lat && f.predicted_lon),
        getPath: (d: any) => [[d.lon, d.lat], [d.predicted_lon, d.predicted_lat]],
    ...
    // 4. Flight Icons (IconLayer) with click handler
    ...
    layers.push(new IconLayer({
      ...
      sizeScale: 1,
      onClick: (info: any) => {
        if (info.object && this.onRadarFlightClickCallback) {
          this.onRadarFlightClickCallback(info.object);
        }
      },
    }));
  ...
  private createLayerToggles(): void {
    ...
    const _wmKey = getSecretState('SKYGUARD_API_KEY').present;
    ...
    // Author badge removed for airspace monitoring version
  ...
  public setOnRadarFlightClick(callback: (flight: any) => void): void {
    this.onRadarFlightClickCallback = callback;
  }
